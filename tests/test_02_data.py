...
def test_charged_pion_2023(api):
    pion = api.get('S008/2023')
    assert isinstance(pion, PdgParticle)
    assert pion.pdgid == 'S008/2023'
    assert pion.edition == '2023'
...
def test_charged_pion_mass_2023(api):
    m = api.get('S008M/2023')
    assert m._count_data_entries('S008m', 2023) == 2
    assert isinstance(m, PdgMass)
    assert m.description == 'pi+- MASS'
    assert m.n_summary_table_values() == 1
    assert m.get_parent_pdgid() == 'S008/2023'
...
def test_error_2023(api):
    assert api.get('S066M2E/2023').best_summary().error == 0.2964938978517235
    assert api.get('M013.4/2023').best_summary().error == 0.0218379495175522
